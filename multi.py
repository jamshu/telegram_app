...
async def download_chunk(session, url, start, end, output_file, progress_bar):
...
async def download_file_concurrently(url, output_file, file_size, num_connections=4):
    chunk_size = math.ceil(file_size / num_connections)
    progress_bar = tqdm(total=file_size, desc="Downloading", unit='B', unit_scale=True)
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for i in range(num_connections):
            ...
            task = asyncio.create_task(download_chunk(session, url, start, end, output_file, progress_bar))
            tasks.append(task)
